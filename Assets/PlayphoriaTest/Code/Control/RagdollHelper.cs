...
// Taken from here: http://perttuh.blogspot.com/2013/10/unity-mecanim-and-ragdolls.html 
...
public class RagdollHelper : MonoBehaviour
{
        ...
        set
        {
            ...
            else
            {
                if (state == RagdollState.ragdolled)
                {
                    ...
                    ragdolledFeetPosition = 0.5f * (anim.GetBoneTransform(HumanBodyBones.LeftToes).position +
    ...
    {
        //Get an array of child components that are of type Rigidbody
    ...
    void LateUpdate()
    {
        ...
        if (state == RagdollState.blendToAnim)
        {
            if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime)
            {
                ...
                Vector3 meanFeetPosition = 0.5f * (anim.GetBoneTransform(HumanBodyBones.LeftFoot).position +
                ...
                transform.rotation *=
                    Quaternion.FromToRotation(animatedDirection.normalized, ragdolledDirection.normalized);
            }

            //compute the ragdoll blend amount in the range 0...1
            float ragdollBlendAmount = 1.0f - (Time.time - ragdollingEndTime - mecanimToGetUpTransitionTime) /
